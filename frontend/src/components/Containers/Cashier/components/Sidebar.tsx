...
const SidebarCashier: FC = () => {
  ...
  useEffect(() => {
    const getAdmin = async () => {
      const cashier = JSON.parse(localStorage.getItem('kasir') || '{}');

      await axios
        .get('/petugas', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        })
        .then(() => setAdminName(cashier.nama_petugas))
  ...
  // Block Access if Login level is not Cashier
  blockAccess('kasir', router);
    ...
    <>
      <nav className="md:left-0 md:block md:fixed md:top-0 md:bottom-0 md:overflow-y-auto md:flex-row md:flex-nowrap md:overflow-hidden bg-slate-100 dark:bg-slate-800 flex flex-wrap items-center justify-between relative md:w-64 z-20 py-4 px-6">
        <div className="md:flex-col md:items-stretch md:min-h-full md:flex-nowrap px-0 flex flex-wrap items-center justify-between w-full mx-auto">
          <Link href="/cashier/dashboard" legacyBehavior>
            {!adminName ? (
