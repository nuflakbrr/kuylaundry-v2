...
import PackageItemAdmin from '../components/PackageItem';
import axios from '../../../../utils/axios';
import { headerConfig } from '../../../../utils/headerConfig';

const ContainersPackageIndex: FC = () => {
  // Required State
        ...
        <div className="p-10 rounded-md w-full">
          <div className="flex flex-wrap items-center justify-between pb-6">
            ...
            <div className="lg:ml-20 ml-10 space-x-8">
              <Link href="/admin/package/add" legacyBehavior>
                <a className="flex items-center justify-center bg-sky-500 hover:bg-sky-600 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer">
                  <FaFolderPlus className="mr-2" /> Tambah Paket
                </a>
              </Link>
            </div>
          </div>
              ...
              <div className="inline-block min-w-full rounded-lg overflow-hidden">
                {!data.length ? (
                  ...
                  data.map((val: any, ind) => (
                    <PackageItemAdmin
                      id={val.id_paket}
                      packageName={val.nama_paket}
                      packagePrice={val.harga}
                      key={ind}
                    />
                  ))
                )}
...
export default ContainersPackageIndex;
